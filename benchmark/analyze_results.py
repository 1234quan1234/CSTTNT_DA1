...
def get_rastrigin_raw_data(
    ...
    for algo, data in raw_data.items():
        ...
        try:
            ...
            data['error_to_optimum'] = np.abs(data['best_fitness'])  # Optimum = 0
...
def generate_rastrigin_summary(
    ...
    """Generate Rastrigin summary CSV."""
    ...
    for n_items, instance_type, seed in configs:
        ...
        for algo in sorted(raw_data.keys()):
            ...
            row = {
                ...
                'Feasibility_Rate': np.mean(feasibility) * 100,
                'Mean_Time': np.mean(times),
...
def main() -> None:
    ...
    if args.problem in ['knapsack', 'all']:
        ...
        if knapsack_dir.exists():
            ...
            if df is not None:
                logger.info("Knapsack summary generated successfully")
        else:
