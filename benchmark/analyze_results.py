...
def generate_rastrigin_summary(
    ...
    """Generate Rastrigin summary CSV with multi-level success rates."""
    ...
    for config_name in configs:
        ...
        for algo, data in raw_data.items():
            ...
            # NEW: Multi-level success rates
            if 'success_levels' in data:
                for level, level_results in data['success_levels'].items():
                    successes = [lr['success'] for lr in level_results if isinstance(lr, dict)]
                    if successes:
                        sr = np.mean(successes) * 100.0
                        row[f'SR_{level}'] = sr
                        
                        # Hitting times for this level
                        hits = [lr.get('hit_evaluations') for lr in level_results 
                               if isinstance(lr, dict) and lr.get('success')]
                        hits = [h for h in hits if h is not None]
                        row[f'HT_med_{level}'] = float(np.median(hits)) if hits else np.nan
            
            # Backward compatibility: use bronze level for old 'SR_<=tol' columns
            # or compute from errors if success_levels not available
